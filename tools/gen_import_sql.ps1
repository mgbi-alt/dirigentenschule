...
# Aufruf:  powershell -File tools\gen_import_sql.ps1
...
$rows = Import-Csv (Join-Path $root 'hk_hausaufgaben_gesamt-export.csv') -Encoding UTF8
$vals = foreach($r in $rows){
  ...
  "  ($($r.user_raw),$jahr,$kw,$(DateSql $r.date_time),$(NumSql (PNum $r.dirigieren_raw)),$(NumSql (PNum $r.stimmbildung_raw)),$(NumSql (PNum $r.klavier_raw)),$(NumSql (PNum $r.gehoerbildung_raw)))"
}
...
function TestsBlock($file,$fach){
  $rows = Import-Csv (Join-Path $root $file) -Encoding UTF8
  $vals = foreach($r in $rows){
    if($SKIP -contains $r.user_raw){ continue }
    "  ($($r.user_raw),$(StrSql $r.monat),$(NumSql (PNum $r.monat_raw)),$(DateSql $r.date_time),$(NumSql (PNum $r.ergebnis_raw)))"
  }
...
$rows = Import-Csv (Join-Path $root 'hk_harmonielehre-export.csv') -Encoding UTF8
$vals = foreach($r in $rows){
  if($SKIP -contains $r.user_raw){ continue }
  "  ($($r.user_raw),$(NumSql (PNum $r.ha_ueberpruefung_raw)),$(NumSql (PNum $r.hausaufgaben1_raw)),$(NumSql (PNum $r.klausur1_raw)),$(NumSql (PNum $r.klausur2_raw)),$(NumSql (PNum $r.gesamt_raw)),$(StrSql $r.gesamtnote_raw))"
}
...
$out.Add("select p.id,'harmonielehre',v.ha,v.hu,v.k1,v.k2,v.ges,v.note from (values")
$out.Add(($vals -join ",`n"))
